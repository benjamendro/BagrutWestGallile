...
function computeAggregated(filtered) {
    ...
    filtered.forEach(d => {
        if (!schoolMap[d.school]) {
            schoolMap[d.school] = {
                ...
                years: [], weightedScoreSum: 0, totalExaminees: 0
            };
        }
        const s = schoolMap[d.school];
        s.years.push(d.year);
        s.weightedScoreSum += d.score * d.examinees;
        s.totalExaminees += d.examinees;
    });
    ...
    return Object.values(schoolMap).map(s => ({
        ...
        sector: s.sector,
        years: s.years.sort(),
        avgScore: s.totalExaminees > 0 ? +(s.weightedScoreSum / s.totalExaminees).toFixed(2) : 0,
        avgExaminees: +(s.totalExaminees / s.years.length).toFixed(2)
    }));
}
...
function renderCharts(aggregated, filtered, subject, units, latestYear, totalLatest, natTotal) {
    ...
    // --- Chart 2: Year trend (total examinees per year) ---
    const yearMap = {};
    filtered.forEach(d => { yearMap[d.year] = (yearMap[d.year] || 0) + d.examinees; });
    const years = Object.keys(yearMap).map(Number).sort();

    // Also get national trend
    const natYearMap = {};
    ...
    if (Object.keys(natYearMap).length > 0) {
        datasets.push({
            label: 'נבחנים ארצי',
            data: years.map(y => natYearMap[y] || null),
    ...
    document.getElementById('chart2-title').textContent = `מגמת מספר נבחנים ב${subject} ${units} יח"ל לאורך השנים`;
    activeCharts.yearTrend = new Chart(document.getElementById('yearTrendChart'), {
        ...
        options: {
            responsive: true,
            scales: {
                y: { beginAtZero: true, title: { display: true, text: 'נבחנים באשכול' } },
                ...(Object.keys(natYearMap).length > 0 ? {
                    y1: { position: 'left', beginAtZero: true, title: { display: true, text: 'נבחנים ארצי' }, grid: { drawOnChartArea: false } }
                } : {})
            },
